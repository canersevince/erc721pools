...
contract('NFTStakeContract', (accounts) => {
    ...
    it('should unstake and restake 2nd time', async () => {
        await NFTStakeContract.leaveStaking(0, tokens, {from: deployer})
        ...
        let advtime = 7000
        await helper.advanceTime(advtime, NFTStake.web3)
        // console.log('CLAIMABLE:', await NFTStakeContract.calculateRewards(0, tokens,((new Date()).getTime() / 1000)))


        await NFTStakeContract.claimReward(0, tokens, {from: deployer})
    ...
    it('should unstake and restake 3rd time', async () => {
        await NFTStakeContract.leaveStaking(0, tokens, {from: deployer})
        let afterBalance = await ERC20Contract.balanceOf(deployer)
    ...
    it('should not earn more and cannot stake anymore ', async () => {
        await NFTStakeContract.leaveStaking(0, tokens, {from: deployer})
        let afterBalance = await ERC20Contract.balanceOf(deployer)
        ...
        const myStakes2 = await NFTStakeContract.getStakes(0, tokens)
        console.log(myStakes2)
        console.log(await NFTContract.ownerOf(0), deployer)
