...
uint public constant MAX_HATCHIES = 40001;
bool public hasSaleStarted = false;

string public METADATA_PROVENANCE_HASH = "";

constructor() ERC721("HatchyPocket", "HATCHIE")  {
setBaseURI("https://hatchypocket.com/api/token/");
...
require(totalSupply() < MAX_HATCHIES, "Sale has already ended");

uint currentSupply = totalSupply();
if (currentSupply >= 40000) {
return 100000000000000000000;
} else if (currentSupply >= 39990) {
return 10000000000000000000;
} else if (currentSupply >= 39900) {
return 6000000000000000000;
} else if (currentSupply >= 39000) {
return 3000000000000000000;
}  else if (currentSupply >= 38000) {
return 2500000000000000000;
} else if (currentSupply >= 37000) {
return 2000000000000000000;
} else if (currentSupply >= 36000) {
return 1550000000000000000;
} else if (currentSupply >= 32000) {
return 1390000000000000000;
} else if (currentSupply >= 28000) {
return 1270000000000000000;
} else if (currentSupply >= 24000) {
return 1150000000000000000;
...
return 870000000000000000;
} else if (currentSupply >= 8000) {
return 790000000000000000;
...
function claimHatchie(uint256 maxHatchies) public payable {
require(totalSupply() < MAX_HATCHIES, "Sale has already ended");
require(maxHatchies > 0 && maxHatchies <= 20, "You can claim minimum 1, maximum 20 hatchies");
require(totalSupply().add(maxHatchies) <= MAX_HATCHIES, "Exceeds MAX_HATCHIES");
require(msg.value >= calculatePrice().mul(maxHatchies), "Ether value sent is below the price");

for (uint i = 0; i < maxHatchies; i++) {
...
_safeMint(address(msg.sender), 0);
_safeMint(address(msg.sender), 1);
_safeMint(address(msg.sender), 2);
_safeMint(address(msg.sender), 3);
_safeMint(address(msg.sender), 4);
_safeMint(address(msg.sender), 5);
}
